...
**Predictable re-generation.** Running `npx openapi-gen` again overwrites `models.ts`, `client.ts`, `client-config.ts`, and `index.ts`. Your `schemas.ts` is never overwritten.
...
**OpenAPI 3.0 `nullable` not yet normalized.** OpenAPI 3.0.x is supported on a best-effort basis. The `nullable: true` construct is not yet normalized to 3.1 `type: [..., "null"]` semantics. This is planned. For now, 3.1 specs get the best coverage.
