...
**Method name derivation:** method names come from `operationId` in your spec (sanitized to camelCase). When `operationId` is absent, the generator falls back to a `verb + path` heuristic: `GET /pets/{id}` becomes `getPetsById`, `POST /users` becomes `createUsers`.
