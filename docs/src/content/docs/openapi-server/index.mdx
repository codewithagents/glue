...
<Aside type="note">
`@codewithagents/openapi-server` requires `@codewithagents/openapi-gen`. Run `openapi-gen` first so `models.ts` exists before `service.ts` imports from it.
</Aside>
...
<Aside type="tip">
The schema naming convention is `${TypeName}Schema`. For a request body type `CreatePetRequest`, the generator looks for `CreatePetRequestSchema` in the schema file. Unmatched schemas trigger a warning to stderr; builds still succeed.
</Aside>

<Aside type="note">
**Same schemas, both sides of the wire.** `@codewithagents/openapi-gen` validates outgoing requests in the browser. `@codewithagents/openapi-server` validates incoming requests on the server. One `schemas.ts`, one source of truth.
</Aside>
