...
| `output` | `string` | Yes | — | Directory to write the generated files |
| `input_schema` | `string` | No | — | Path to a user-owned Zod schema file. Bootstrapped on first run if the file does not exist; never overwritten after that |
...
<Aside type="tip">
The `.strip()` call before sending removes any UI-only fields you added to the schema for forms or multi-step wizards, so they never reach the API.
</Aside>

<Aside type="note">
If a schema exists in `schemas.ts` but has no matching component in the spec (or vice versa), the generator prints a drift warning to stderr. Your build still succeeds; the warning is advisory.
</Aside>
