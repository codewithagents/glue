...
Call `configureClient` once at startup. Calling it a second time merges into the existing config rather than replacing it. The `token` field accepts a string or an async function, called per request, so JWT refresh flows work automatically.
...
Ensure your config includes `input_schema` pointing at an existing `schemas.ts` file. On first run with `input_schema` set but no file present, the generator bootstraps the file and also re-generates `models.ts` and `client.ts` with Zod integration in a single pass. If the file did not exist on the first run, run the generator a second time after the file has been created.
