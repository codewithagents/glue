...
Parsers are tried in a fixed order and the **first match wins**: RFC 7807 errors map, then Spring Boot array, then flat object, then RFC 9457 `detail`. A body that contains both an `errors` map and a `detail` field uses the `errors` map; the `detail` field is ignored.
