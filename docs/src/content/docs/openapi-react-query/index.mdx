...
Your own `onSuccess` callback (if provided in `options`) is called **after** auto-invalidation runs. The generated `onSuccess` composes your callback rather than replacing it:
...
The built-in guard is `enabled: id != null && (options?.enabled ?? true)`. If you explicitly pass `enabled: true` in `options`, that overrides the guard and the hook will fire immediately with a non-null assertion on the id. Omit `enabled` (or pass `enabled: undefined`) to keep the nullish guard active.
